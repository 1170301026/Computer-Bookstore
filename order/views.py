...
def order_place(request):
    ...
    for id in books_ids:
        ...
        count = conn.hget(cart_key, id).decode()
        books.count = count
        # 计算商品的小计
...
def order_pay(request):
    ...
    app_private_key_path = 'D:/Python/Bookstore/order/app_private_key.pem'
    alipay_public_key_path = 'D:/Python/Bookstore/order/app_public_key.pem'
    ...
    # 和支付宝进行交互
    aliPay = AliPay(
        appid="2016101500690269",
        app_notify_url=None,  # 默认回调url
        app_private_key_string=app_private_key_string,
        alipay_public_key_string=alipay_public_key_string,  # 支付宝公钥，验证支付宝
        sign_type="RSA2",  # RSA或者RSA2
        debug=True,  # 默认False
    )
    # 电脑网站支付，需要跳转到htts://openapi.alipaydev.com/gateway.do?+order_stringg
    total_pay = order.total_price + order.transit_price  # decimal
    order_string = aliPay.api_alipay_trade_page_pay(out_trade_no=order_id,
                                                    total_amount=str(total_pay),
                                                    subject='求之书店%s' % order_id,
                                                    return_url=None,
                                                    notify_url=None)
    # 返回应答
